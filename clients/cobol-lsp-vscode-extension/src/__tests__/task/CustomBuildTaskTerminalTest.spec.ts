...
import { CobolCompileJobDefinition } from "../../task/providers/CompileTaskProvider";
import {CompileService} from "../../task/taskServices/CompileService";
import { CRLF, CustomBuildTaskTerminal } from "../../task/terminal/CustomBuildTaskTerminal";
...
describe("CustomBuildTaskTerminal Test", () => {
    ...
    it("initializes CustomBuildTaskTerminal properly", () => {
        ...
        (taskTerminal as any).compileService.doCompile = mockCompileFunction;
    ...
    it("returns with error when ZE is not installed", async () => {
        ...
        await (taskTerminal as any).compileService.doCompile().catch(e => {
    ...
    it("return with error when jobCard is not provided", async () => {
        ...
        await (incompleteTask as any).compileService.doCompile().catch(e => {
    ...
    it("checks compile works when all data is provided", async () => {
        ...
        const mockCompileService = new CompileService((taskTerminal as any).writeEmitter, (taskTerminal as any).closeEmitter, (taskTerminal as any).definition, (taskTerminal as any).documentText);
        (taskTerminal as any).compileService = mockCompileService;
        (mockCompileService as any).compileCobolDocument = jest.fn().mockReturnValue("TEST");
        await expect((taskTerminal as any).compileService.doCompile()).resolves.not.toThrowError();
    });
    ...
    it("checks proper flow of task", async () => {
        ...
        const task = new CustomBuildTaskTerminal(docText as any, {
            ...
            type: "testTask",
            listingFile: {ddname: "SYSPRINT", spoolid : 12, stepname: "XYZ", procstep: ""}
        });
        ...
        (task as any).compileService = new CompileService((task as any).writeEmitter, (task as any).closeEmitter, (task as any).definition, (task as any).documentText);
        await (task as any).compileService.compileCobolDocument(vscode.extensions.getExtension("Zowe.vscode-extension-for-zowe")?.exports, loadedProfile);
        expect(writterEvent).toBeCalledTimes(7);
    });
    ...
    it("checks proper JCL is formed for compile job", async () => {
        ...
        const JCL = (CobolCompileJCLProvider as any).getCompileJobJCL(definition, "SAMPLE TEXT");
