...
export class CompileService {
    ...
    private async compileCobolDocument(zoweExplorerApi: ZoweExplorerApi.IApiRegisterClient, loadedProfile: IProfileLoaded) {
        ...
        if (`${job.retcode}` !== "JCL ERROR") {
            ...
            if (this.definition.listFiles) {
                await this.displayService.displaySpoolFiles(spoolFiles, jesApi, this.jobContext);
            }
            if (this.definition.stepSummary) {
                const jobSummary = await this.displayService.getMessageFromJesMsg(spoolFiles
                    .filter(file => file.ddname === "JESYSMSG")[0], jesApi);
            ...
            this.writeEmitter.fire(`\x1b[32mAnalysing output...${CRLF}`);
    ...
    private async getJobStatus(counter: number, delay: number, job, jesApi: ZoweExplorerApi.IJes, submitJob) {
        counter++;
        if (counter % 3 === 0) {
            const n = counter / 3;
            delay = delay + n * 1000;
    ...
    public generateUri(spoolName: string, spoolid?: number, jobid?: string, jobname?: string): vscode.Uri {
        ...
        return vscode.Uri.from({ scheme: SpoolContentProvider.scheme, path: `${stepContext.jobid}/${stepContext.jobname}/${spoolid || stepContext.spoolid}/${spoolName}` });
    }
