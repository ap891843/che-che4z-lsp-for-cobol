...
public class CobolVisitor extends CobolParserBaseVisitor<List<Node>> {
  ...
  public List<Node> visitDeleteStatement(DeleteStatementContext ctx) {
    Optional<Locality> locality1 = retrieveLocality(ctx.deleteFilenameClause().fileName());
    if (locality1.isPresent()) {
      VariableNameAndLocality fileNameLocality =
          new VariableNameAndLocality(
      ...
      return addTreeNode(
          ctx,
          locality ->
              new FileOperationStatementNode(
                  locality, fileNameLocality, NodeType.DELETE_STATEMENT));
    } else {
